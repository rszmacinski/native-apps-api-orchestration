...
class TestGenericOrchestrationService(testExecutorFactory: ExecutorFactory, override val maxServiceCalls: Int, override val maxEventCalls: Int) extends LiveOrchestrationService {
  override def buildAndExecute(orchestrationRequest: OrchestrationRequest, nino: Option[String] = None, journeyId: Option[String])(implicit hc: HeaderCarrier, ex: ExecutionContext): Future[OrchestrationResponse] = testExecutorFactory.buildAndExecute(orchestrationRequest, nino, journeyId)
  override val auditConnector: AuditConnector = MicroserviceAuditConnector
